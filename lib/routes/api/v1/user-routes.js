...
exports.register = (server, options, next) => {
    ...
    server.dependency(['hapi-sequelizejs'], internals.after);
    ...
    server.route({
        ...
        config: {
            ...
            handler: (request, reply) => {
                const db = request.getDb('db');
                
                // db.getModel('User').create({
                //     "username": "tbear",
                ...
                db.getModel('User').findAll({ raw: true }).then(users => {
                        // console.log(users);
                        return reply(users);
                    });
                ...
                // db.getModel('User').findById(1).then(users => {
                //     return reply(users.dataValues);
                // });                
            }
        }
    });
